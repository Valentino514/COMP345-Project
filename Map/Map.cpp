...
Map::~Map() {
    ...
    delete Territories; // Free the map itself
    delete Continents;
}
...
bool Map::isMapConnected() {
    ...
    while (!stack.empty()) {
        ...
        if (visited.find(current) == visited.end()) {   // Mark territory as visited
            ...
            for (Territory* adjacent : *(current->getAdjacentTerritories())) {  // Dereference the pointer
...
bool Map::areContinentsConnected() {
    ...
    for (const auto& pair : *Continents) {  // Dereference Continents pointer
        ...
        for (const auto& territoryPair : *Territories) {  // Dereference Territories pointer
        ...
        while (!stack.empty()) {
            ...
            if (visited.find(current) == visited.end()) {   // Mark territory as visited
                ...
                for (Territory* adjacent : *(current->getAdjacentTerritories())) {  // Dereference pointer
...
bool Map::eachTerritoryHasOneContinent() {
    ...
    for (const auto& pair : *Territories) {  // Dereference Territories pointer
...
void Territory::addAdjacentTerritory(Territory* adjacent) {
    AdjTerritories->push_back(adjacent); // Dereference and modify
}
...
Territory::Territory(std::string* name, std::string* continent, std::vector<Territory*>* territories, Player* player, int* army)
    : TerritoryName(new std::string(*name)), 
      ...
      AdjTerritories(new std::vector<Territory*>(*territories)),  // Deep copy of territories
...
Territory::Territory(const Territory& other) {
    ...
    AdjTerritories = new std::vector<Territory*>(*other.AdjTerritories); // Deep copy of adjacent territories
...
Territory& Territory::operator=(const Territory& other) {
    ...
    TerritoryName = new std::string(*other.TerritoryName);
    continent = new std::string(*other.continent);
    AdjTerritories = new std::vector<Territory*>(*other.AdjTerritories); // Deep copy
    armyAmount = new int(*other.armyAmount);
    territoryOwner = other.territoryOwner ? new Player(*other.territoryOwner) : nullptr;
    return *this;
}
...
// Helper Method to remove preceding spaces
...
// Updated MapLoader::loadMap function using the splitLine helper function
Map* MapLoader::loadMap(const std::string& path) {
    ...
    // Construct the regex once
    std::regex re(",\\s*");  // Split the input line by commas and optional spaces
